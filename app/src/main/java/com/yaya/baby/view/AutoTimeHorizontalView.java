...
/**
 * @author Darcy
 * @deprecated 首页的横向滑动选择时间的自定义控件
 */

public class AutoTimeHorizontalView extends RecyclerView {
    /**
    ...
    public AutoTimeHorizontalView(Context context) {
        super(context);
    }

    public AutoTimeHorizontalView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public AutoTimeHorizontalView(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    /**
     * 设置初始化时选中的位置,该方法必须在{@link AutoTimeHorizontalView#setAdapter(android.support.v7.widget.RecyclerView.Adapter) }之前调用
     *
    ...
    /**
     * 设置每次显示多少个item,该方法必须在{@link AutoTimeHorizontalView#setAdapter(android.support.v7.widget.RecyclerView.Adapter) }之前调用
     *
    ...
    class WrapperAdapter extends RecyclerView.Adapter {
        ...
        public WrapperAdapter(Adapter adapter, Context context, int itemCount) {
            ...
            if (adapter instanceof IAutoLocateHorizontalView) {
                ...
                throw new RuntimeException(adapter.getClass().getSimpleName() + "!");
            }
        }
